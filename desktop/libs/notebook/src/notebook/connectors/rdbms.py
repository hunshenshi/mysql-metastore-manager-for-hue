...
class RdbmsApi(Api):
  ...
  def autocomplete(self, snippet, database=None, table=None, column=None, nested=None):
    ...
    if database is None:
      ...
      tables_meta = []
      # add by sunzhiwei for fix a bug
      db.use(database)
      for t in assist.get_comment(database):
        tables_meta.append({'name': t['name'], 'type': 'Table', 'comment': t['comment']})
      response['tables_meta'] = tables_meta
    else:
      columns = assist.get_comment(database, table)
...
class Assist():
  ...
  # add by szw for execute a sql
  def get_comment(self, database, table=None):
    return self.db.get_comment(database, table)
