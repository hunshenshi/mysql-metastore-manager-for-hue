...
(function (root, factory) {
  ...
  MetastoreDatabase.prototype.load = function (callback, optimizerEnabled) {
    ...
    self.loading(true);
    // modify sourceType by sunzhiwei for mysql metatable
    ...
    // modify sourceType by sunzhiwei for mysql metatable
    // get_database_metadata in views
    $.getJSON('/rdbMetaStore/databases/' + self.name + '/metadata', function (data) {
      if (data && data.status == 0) {
    ...
    // modify sourceType by sunzhiwei for mysql metatable
    // cache lastdb
    $.totalStorage('hue.rdbMetaStore.lastdb', self.name);
  };
  ...
  MetastoreDatabase.prototype.setTable = function (metastoreTable, callback) {
    var self = this;
    // modify sourceType by sunzhiwei for mysql metatable
    huePubSub.publish('rdbMetaStore.scroll.to.top');
  ...
  function MetastoreTable(options) {
    ...
    self.comment.subscribe(function (newValue) {
      ...
      $.post('/rdbMetaStore/table/' + self.database.name + '/' + self.name + '/alter', {
  ...
  MetastoreTable.prototype.showImportData = function () {
    var self = this;
    $.get('/rdbMetaStore/table/' + self.database.name + '/' + self.name + '/load', function (data) {
  ...
  function MetastoreColumn(options) {
    ...
    self.comment.subscribe(function (newValue) {
      // modify sourceType by sunzhiwei for mysql metatable
      // alter column's comment
      $.post('/rdbMetaStore/table/' + self.table.database.name + '/' + self.table.name + '/alter_column', {
        column: self.name(),
        type: self.type,
        comment: newValue
      }, function (data) {
  ...
  function MetastoreViewModel(options) {
    ...
    var setDatabaseByName = function (databaseName, callback) {
      // modify sourceType by sunzhiwei for mysql metatable
      // default database is test in mysql
      if (databaseName === '') {
        databaseName = $.totalStorage('hue.rdbMetaStore.lastdb') || 'test';
      }
    ...
    huePubSub.subscribe("assist.table.selected", function (tableDef) {
      loadTableDef(tableDef, function () {
        huePubSub.publish('rdbMetaStore.url.change')
      });
    });
    ...
    // modify sourceType by sunzhiwei for mysql metatable
    huePubSub.subscribe('rdbMetaStore.url.change', function () {
      // 
      if (self.database() && self.database().table()) {
        hueUtils.changeURL('/rdbMetaStore/table/' + self.database().name + '/' + self.database().table().name);
      }
      else if (self.database()) {
        // show_tables 
        hueUtils.changeURL('/rdbMetaStore/tables/' + self.database().name);
      }
      else {
        hueUtils.changeURL('/rdbMetaStore/databases');
      }
    });
