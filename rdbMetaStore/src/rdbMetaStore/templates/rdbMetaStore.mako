...
${ commonheader(_("rdbMetaStore"), app_name, user) | n,unicode }
${ components.menubar() }
...
<link rel="stylesheet" href="${ static('rdbMetaStore/css/rdbMetaStore.css') }" type="text/css">
...
<script type="text/html" id="metastore-databases">
  ...
  <table id="databasesTable" class="table table-condensed datatables" style="margin-bottom: 10px" data-bind="visible: filteredDatabases().length > 0">
    ...
    <tr>
      ...
      <td>
        <a href="javascript: void(0);" data-bind="text: name, click: function () { $parent.setDatabase($data, function(){ huePubSub.publish('rdbMetaStore.url.change'); }) }"></a>
      </td>
    </tr>
    </tbody>
  </table>
        ...
        <table id="tablesTable" class="table table-striped table-condensed table-nowrap" style="margin-bottom: 10px; width: 100%" data-bind="visible: filteredTables().length > 0">
          ...
          <tbody data-bind="hueach: {data: filteredTables, itemHeight: 29, scrollable: '.right-panel', scrollableOffset: 277}">
            <tr>
              ...
              <td>
                <a class="tableLink" href="javascript:void(0);" data-bind="text: name, click: function() { $parent.setTable($data, function(){ huePubSub.publish('rdbMetaStore.url.change'); }) }"></a>
              </td>
...
<script type="text/html" id="metastore-databases-actions">
  <div class="inline-block pull-right">
    <a class="inactive-action" href="javascript:void(0)" data-bind="click: function () { huePubSub.publish('assist.db.refresh', 'mysql'); }"><i class="pointer fa fa-refresh" data-bind="css: { 'fa-spin blue' : $root.reloading }" title="${_('Refresh')}"></i></a>
    <!--
    % if has_write_access:
...
<script type="text/html" id="metastore-partitions-tab">
  ...
  <!-- /ko -->
  <a data-bind="attr: { 'href': '/rdbMetaStore/table/' + database.name + '/' + name + '/partitions' }">${ _('View all') }</a>
</script>
      ...
      <!-- ko with: $root.database().table().optimizerDetails().table_donut -->
        <table data-bind="visible: joinedtables().length > 0" class="table table-condensed">
          ...
          <tr>
            ...
            <td><a data-bind="text: tableName, attr: { href: '/rdbMetaStore/table/' + $root.database().name + '/' + tableName() }"</a></td>
  ...
  <div id="dropSingleTable" class="modal hide fade">
    <form method="POST">
      ...
      <div class="modal-footer">
        ...
        <input type="submit" data-bind="click: function (vm, e) { var $form = $(e.target).parents('form'); $form.attr('action', '/rdbMetaStore/tables/drop/' + vm.database().name); return true; }" class="btn btn-danger" value="${_('Yes, drop this table')}"/>
      </div>
    </form>
  </div>
...
<script type="text/javascript" charset="utf-8">
  ...
  require([
    'knockout',
    'rdbMetaStore/js/rdbMetaStore.ko',
    'ko.charts',
    'assistPanel',
    'tableStats',
    'knockout-mapping',
    ...
    $(document).ready(function () {
      ...
      huePubSub.subscribe('rdbMetaStore.scroll.to.top', function () {
      ...
      if (location.getParameter('refresh') === 'true') {
        huePubSub.publish('assist.db.refresh', 'mysql');
        hueUtils.replaceURL('?');
      }
