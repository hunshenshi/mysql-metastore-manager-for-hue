...
from rdbMetaStore.forms import LoadDataForm, DbForm
from rdbMetaStore.settings import DJANGO_APPS
...
def index(request):
  return redirect(reverse('rdbMetaStore:show_tables', kwargs={'database': ''}))
...
#  search_filter = request.GET.get('filter', '')
  ...
  return render("rdbMetaStore.mako", request, {
    'breadcrumbs': [],
    'database': None,
    'databases': databases,
    'partitions': [],
...
#  db = dbms.get(request.user)
  ...
  try:
#    print "get_database_metadata " + database
#    db_metadata = db.get_database(database)
#    db.use(database)
#    db_metadata = db.get_tables(database)
    response['status'] = 0
#    print db_metadata
...
Table Views
"""
# ${ url('rdbMetastore:show_tables', database=database) }
def show_tables(request, server=None, database=None):
#  print server
#  print "+++++++++++++++++++++++++++"
#  print database 
  # just for mysql   
  ...
  if database is None:
    database = 'test' # Assume always 'test'
...
#        'search_filter': search_filter
    ...
    resp = render("rdbMetaStore.mako", request, {
    'breadcrumbs': [],
    'database': None,
    'partitions': [],
    'has_write_access': has_write_access(request.user),
...
#  print "describe_table query_server" 
#  print query_server
  db = dbms.get(request.user, query_server)
...
#      partitions = [_massage_partition(database, table, partition) for partition in db.get_partitions(database, table)]
    ...
    return render(renderable, request, {
      'breadcrumbs': [{
          'name': database,
          'url': reverse('rdbMetaStore:show_tables', kwargs={'database': database})
        }, {
#          'name': str(table.name),
          'name': str(table),
          'url': reverse('rdbMetaStore:describe_table', kwargs={'database': database, 'table': table}) #table.name
        },
      ],
      'table': table,
...
#  db = dbms.get(request.user)
  ...
  try:
    ...
    #print " view alter_table_comment " + comment
    table_obj = db.alter_table_comment(database, table, comment)
...
#  db = dbms.get(request.user)
  ...
  try:
    ...
    if type is None:
      raise PopupException(_('alter_column requires a column type parameter'))

    column_obj = db.alter_column_comment(database, table, column, type, comment)
